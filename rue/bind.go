...
// ContractBackend implements bind.ContractBackend with direct calls to Rue
// internals to support operating on contracts within subprotocols like rue and
// swarm.
//
// Internally this backend uses the already exposed API endpoints of the Rue
// object. These should be rewritten to internal Go method calls when the Go API
// is refactored to support a clean library use.
type ContractBackend struct {
	eapi  *rueapi.PublicRueAPI        // Wrapper around the Rue object to access metadata
...
func NewContractBackend(apiBackend rueapi.Backend) *ContractBackend {
	return &ContractBackend{
		eapi:  rueapi.NewPublicRueAPI(apiBackend),
...
// ContractCall implements bind.ContractCaller executing an Rue contract
// call with the specified data as the input. The pending flag requests execution
// against the pending block, not the stable head of the chain.
func (b *ContractBackend) CallContract(ctx context.Context, msg rue.CallMsg, blockNum *big.Int) ([]byte, error) {
...
// call with the specified data as the input. The pending flag requests execution
// against the pending block, not the stable head of the chain.
func (b *ContractBackend) PendingCallContract(ctx context.Context, msg rue.CallMsg) ([]byte, error) {
	out, err := b.bcapi.Call(ctx, toCallArgs(msg), rpc.PendingBlockNumber)
	return out, err
}

func toCallArgs(msg rue.CallMsg) rueapi.CallArgs {
...
func (b *ContractBackend) EstimateGas(ctx context.Context, msg rue.CallMsg) (*big.Int, error) {
