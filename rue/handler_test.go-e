...
func testGetNodeData(t *testing.T, protocol int) {
	...
	generator := func(i int, block *core.BlockGen) {
		switch i {
		case 0:
			// In block 1, the test bank sends account #1 some rue.
			...
			block.AddTx(tx)
		case 1:
			// In block 2, the test bank sends some more rue to account #1.
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*ruedb.MemDatabase).Keys() {
	...
	statedb, _ := ruedb.NewMemDatabase()
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	peer, _ := newTestPeer("peer", rue63, pm, true)
	defer peer.close()
	...
	if !timeout {
		blocks, _ := core.GenerateChain(&params.ChainConfig{}, genesis, ruehash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
			if remoteForked {
