...
import (
	...
	"github.com/Rue-Foundation/go-rue/rue/tracers"
...
type ephemeralDatabase struct {
	diskdb ruedb.Database     // Persistent disk database to fall back to with reads
	memdb  *ruedb.MemDatabase // Ephemeral memory database for primary reads and writes
}
...
func (db *ephemeralDatabase) NewBatch() ruedb.Batch {
...
func (db *ephemeralDatabase) Prune(root common.Hash) {
	...
	db.memdb, _ = ruedb.NewMemDatabaseWithCap(db.memdb.Len())
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.rue.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.rue.blockchain.CurrentBlock()
	default:
		from = api.rue.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.rue.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.rue.blockchain.CurrentBlock()
	default:
		to = api.rue.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	memdb, _ := ruedb.NewMemDatabase()
	db := &ephemeralDatabase{
		diskdb: api.rue.ChainDb(),
		memdb:  memdb,
	}
	if number := start.NumberU64(); number > 0 {
		start = api.rue.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					vmctx := core.NewEVMContext(msg, task.block.Header(), api.rue.blockchain, nil)
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.rue.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.rue.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.rue.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.rue.blockchain.CurrentBlock()
	default:
		block = api.rue.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	block := api.rue.blockchain.GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.rue.engine.VerifyHeader(api.rue.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.rue.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				vmctx := core.NewEVMContext(msg, block.Header(), api.rue.blockchain, nil)
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	for i := uint64(0); i < reexec; i++ {
		block = api.rue.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.rue.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	tx, blockHash, _, index := core.GetTransaction(api.rue.ChainDb(), hash)
	if tx == nil {
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	case *vm.StructLogger:
		return &rueapi.ExecutionResult{
			Gas:         gas,
			Failed:      failed,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  rueapi.FormatLogs(tracer.StructLogs()),
		}, nil
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	// Create the parent state database
	block := api.rue.blockchain.GetBlockByHash(blockHash)
	if block == nil {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewEVMContext(msg, block.Header(), api.rue.blockchain, nil)
		if idx == txIndex {
