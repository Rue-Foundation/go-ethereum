...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Rue address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Rue transaction after having the user validate the parameters
...
type ledgerDriver struct {
	...
	browser bool          // Flag whrue the Ledger is in browser mode (reply channel mismatch)
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Rue app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Rue app's version, will fail prior to v1.0.2
...
// ledgerVersion retrieves the current version of the Rue wallet app running
...
// ledgerDerive retrieves the currently active Rue address from a Ledger
...
//   Rue address length | 1 byte
//   Rue address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Rue hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Rue address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whrue legacy or EIP155 signing was requeste
	var (
		txrlp []byte
		err   error
	)
