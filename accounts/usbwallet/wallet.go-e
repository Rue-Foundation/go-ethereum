...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Rue
...
// whrue they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Rue account
...
// Contains implements accounts.Wallet, returning whrue a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain rue.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
