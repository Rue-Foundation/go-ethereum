...
// faucet is a Rue faucet backed by a light client.
package main
...
var (
	...
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Rues to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Rue and the faucet")
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Rues", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *ruePortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Rue address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Rue light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Rue protocol stack
	client *rueclient.Client   // Client connection to the Rue chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Rue light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := rue.DefaultConfig
	...
	// Assemble the ruestats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightRue
			ctx.Service(&serv)
			return ruestats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := rueclient.NewClient(api)
...
// close terminates the Rue connection and tears down the faucet.
...
// apiHandler handles requests for Rue grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Rue address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somrueing funky happened, please open an issue at https://github.com/Rue-Foundation/go-rue/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and Rue address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Rue addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Rue address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Rue address and profile URL.
...
// returning the username, avatar URL and Rue address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Rue address,
