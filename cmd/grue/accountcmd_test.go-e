...
func TestAccountListEmpty(t *testing.T) {
	grue := runGrue(t, "account", "list")
	grue.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	grue := runGrue(t, "account", "list", "--datadir", datadir)
	defer grue.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	grue := runGrue(t, "account", "new", "--lightkdf")
	defer grue.ExpectExit()
	grue.Expect(`
...
`)
	grue.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	grue := runGrue(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer grue.ExpectExit()
	grue.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(grue.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(grue.StderrText(), m) {
...
// https://github.com/Rue-Foundation/go-rue/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	grue.SetTemplateFunc("keypath", func(file string) string {
