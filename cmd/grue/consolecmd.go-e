...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/Rue-Foundation/go-rue/wiki/Javascipt-Console.
This command allows to open a console on a running grue node.`,
	}
...
// localConsole starts a new grue node, attaching a JavaScript console to it at the
// same time.
...
// remoteConsole will connect to a remote grue instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	// Attach to a remotely running grue instance and start the JavaScript console
...
// for "grue attach" and "grue monitor" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with grue < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
...
// ephemeralConsole starts a new grue node, attaches an ephemeral JavaScript
