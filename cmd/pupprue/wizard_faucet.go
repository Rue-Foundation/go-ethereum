...
func (w *wizard) deployFaucet() {
	...
	if err != nil {
		infos = &faucetInfos{
			node:    &nodeInfos{portFull: 30304, peersTotal: 25},
			port:    80,
			host:    client.server,
			amount:  1,
			minutes: 1440,
	...
	fmt.Println()
	fmt.Printf("How many Rues to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.ruestats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.ruestats = w.readString() + ":" + w.conf.ruestats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.ruestats)
		infos.node.ruestats = w.readDefaultString(infos.node.ruestats) + ":" + w.conf.ruestats
	}
