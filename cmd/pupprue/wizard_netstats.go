...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for ruestats availability")
	if infos, err := checkRuestats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["ruestats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["ruestats"] = infos.Report()
		ruestats = infos.config
	}
