...
// config contains all the configurations needed by pupprue that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootFull  []string // Bootnodes to always connect to by full nodes
	bootLight []string // Bootnodes to always connect to by light nodes
	ruestats  string   // Ruestats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save pupprue configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Rue services known to be running on servers
...
// converts it to an Rue address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by ruestats, which compares IPs textually, not by value.
