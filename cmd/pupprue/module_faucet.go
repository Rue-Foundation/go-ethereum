...
FROM rue/client-go:alltools-latest
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Ruestats":      config.node.ruestats,
		"RuePort":       config.node.portFull,
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"RuePort":       config.node.portFull,
		"RueName":       config.node.ruestats[:strings.Index(config.node.ruestats, ":")],
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
		"FaucetAmount":  config.amount,
		"FaucetMinutes": config.minutes,
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Rue listener port":       strconv.Itoa(info.node.portFull),
		"Funding amount (base tier)":   fmt.Sprintf("%d Rues", info.amount),
		"Funding cooldown (base tier)": fmt.Sprintf("%d mins", info.minutes),
		"Funding tiers":                strconv.Itoa(info.tiers),
		"Captha protection":            fmt.Sprintf("%v", info.captchaToken != ""),
		"Ruestats username":            info.node.ruestats,
	}
	if info.noauth {
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			portFull: infos.portmap[infos.envvars["RUE_PORT"]+"/tcp"],
			ruestats: infos.envvars["RUE_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
