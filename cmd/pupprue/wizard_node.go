...
func (w *wizard) deployNode(boot bool) {
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{portFull: 30304, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{portFull: 30304, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	existed := err == nil
	...
	if w.conf.Genesis.Config.Ruehash != nil && !boot {
		fmt.Println()
		if infos.ruehashdir == "" {
			fmt.Printf("Where should the ruehash mining DAGs be stored on the remote machine?\n")
			infos.ruehashdir = w.readString()
		} else {
			fmt.Printf("Where should the ruehash mining DAGs be stored on the remote machine? (default = %s)\n", infos.ruehashdir)
			infos.ruehashdir = w.readDefaultString(infos.ruehashdir)
		}
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos, nocache); err != nil {
		log.Error("Failed to deploy Rue node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
