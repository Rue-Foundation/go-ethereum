...
// ruestatsComposefile is the docker-compose.yml file required to deploy and
// maintain an ruestats monitoring site.
var ruestatsComposefile = `
version: '2'
services:
  ruestats:
    build: .
    image: {{.Network}}/ruestats{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployRuestats deploys a new ruestats container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployRuestats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(ruestatsDockerfile)).Execute(dockerfile, map[string]interface{}{
	...
	template.Must(template.New("").Parse(ruestatsComposefile)).Execute(composefile, map[string]interface{}{
		"Network": network,
		"Port":    port,
		"Secret":  secret,
		"VHost":   vhost,
	...
	// Build and deploy the ruestats service
	if nocache {
...
// ruestatsInfos is returned from an ruestats status check to allow reporting
// various configuration parameters.
type ruestatsInfos struct {
	host   string
	port   int
	secret string
	config string
...
func (info *ruestatsInfos) Report() map[string]string {
...
// checkRuestats does a health-check against an ruestats server to verify whrue
...
func checkRuestats(client *sshClient, network string) (*ruestatsInfos, error) {
	...
	infos, err := inspectContainer(client, fmt.Sprintf("%s_ruestats_1", network))
	if err != nil {
		return nil, err
	}
	if !infos.running {
	...
	if err = checkPort(host, port); err != nil {
		log.Warn("Ruestats service seems unreachable", "server", host, "port", port, "err", err)
	}
	// Container available, assemble and return the useful infos
	return &ruestatsInfos{
		host:   host,
		port:   port,
		secret: secret,
		config: config,
