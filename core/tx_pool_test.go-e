...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.Rue))
		*c.trigger = false
	}
	return stdb, nil
}
...
// state reset and tests whrue the pending state is in sync with the
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.Rue))
...
// Tests that the transaction limits are enforced the same way irrelevant whrue
