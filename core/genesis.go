...
func SetupGenesisBlock(db ruedb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	if genesis != nil && genesis.Config == nil {
		return params.AllRuehashProtocolChanges, common.Hash{}, errGenesisNoConfig
	}
	...
	// Check whrue the genesis block is already written.
	if genesis != nil {
...
func (g *Genesis) configOrDefault(ghash common.Hash) *params.ChainConfig {
	...
	default:
		return params.AllRuehashProtocolChanges
	}
}
...
func (g *Genesis) Commit(db ruedb.Database) (*types.Block, error) {
	...
	config := g.Config
	if config == nil {
		config = params.AllRuehashProtocolChanges
	}
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db ruedb.Database) *types.Block {
...
func GenesisBlockForTesting(db ruedb.Database, addr common.Address, balance *big.Int) *types.Block {
	g := Genesis{Alloc: GenesisAlloc{addr: {Balance: balance}}}
	return g.MustCommit(db)
}

// DefaultGenesisBlock returns the Rue main net genesis block.
...
// DeveloperGenesisBlock returns the 'grue --dev' genesis block. Note, this must
// be seeded with the
