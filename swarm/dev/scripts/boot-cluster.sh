...
#
# The cluster contains a bootnode, a grue node and multiple swarm nodes, with
...
# static grue configuration
GRUE_IP="192.168.33.3"
GRUE_RPC_PORT="8545"
GRUE_RPC_URL="http://${GRUE_IP}:${GRUE_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the grue and
# swarm nodes
start_bootnode() {
...
# start_grue_node starts a grue node with --datadir pointing at <base-dir>/grue
# and a single, unlocked account with password "grue"
start_grue_node() {
  local dir="${base_dir}/grue"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get grue account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GRUE_IP}"
    --rpcport   "${GRUE_RPC_PORT}"
    --verbosity "6"
  )

  start_node "grue" "${GRUE_IP}" "$(which grue)" ${args[@]}
}
...
create_account() {
  ...
  grue --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
