...
// PublicRueAPI provides an API to access Rue related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicRueAPI struct {
	b Backend
}

// NewPublicRueAPI creates a new Rue protocol API.
func NewPublicRueAPI(b Backend) *PublicRueAPI {
	return &PublicRueAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicRueAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Rue protocol version this node supports
func (s *PublicRueAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicRueAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Rue Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Rue Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Rue ECDSA signature for:
// keccack256("\x19Rue Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/Rue-Foundation/go-rue/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with rue_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Rue Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/Rue-Foundation/go-rue/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Rue signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Rue blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Rue blockchain API.
...
//
// https://github.com/rue/wiki/wiki/JSON-RPC#rue_sign
...
// PublicDebugAPI is the collection of Rue APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", ruehash.SeedHash(number)), nil
}

// PrivateDebugAPI is the collection of Rue APIs exposed over the private
// debugging endpoint.
...
// Version returns the current rue protocol version.
