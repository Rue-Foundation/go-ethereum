...
// Package ruestats implements the network stats reporting service.
package ruestats
...
import (
	...
	"github.com/Rue-Foundation/go-rue/common/mclock"
...
// Service implements an Rue netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	rue    *rue.Rue      // Full Rue service if monitoring a full node
	les    *les.LightRue // Light Rue service if monitoring a light node
...
func New(url string, rueServ *rue.Rue, lesServ *les.LightRue) (*Service, error) {
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // Ruestats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["rue"]; info != nil {
		network = fmt.Sprintf("%d", info.(*rue.RueNodeInfo).Network)
		protocol = fmt.Sprintf("rue/%d", rue.ProtocolVersions[0])
	} else {
		network = fmt.Sprintf("%d", infos.Protocols["les"].(*rue.RueNodeInfo).Network)
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the ruestats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to ruestats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to ruestats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.rue != nil {
		...
		header = block.Header()
		td = s.rue.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to ruestats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	log.Trace("Sending pending transactions to ruestats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.rue != nil {
		...
		sync := s.rue.Downloader().Progress()
		syncing = s.rue.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		price, _ := s.rue.ApiBackend.SuggestPrice(context.Background())
	...
	log.Trace("Sending node details to ruestats")
